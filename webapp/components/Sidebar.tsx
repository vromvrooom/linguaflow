...
export default function Sidebar() {
  ...
  return (
    <>
      {/* ── Desktop sidebar ───────────────────────────────────────────────── */}
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-[260px] flex-col bg-surface border-r border-line shadow-sidebar">
          ...
          {NAV.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-[15px] transition-colors duration-200 ${
                  active
                    ? 'bg-brand-soft text-brand font-semibold'
                    : 'text-dim hover:text-ink hover:bg-paper font-medium'
                }`}
              >
          ...
          {NAV.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm whitespace-nowrap transition-colors duration-200 ${
                  active ? 'bg-brand-soft text-brand font-semibold' : 'text-dim hover:text-ink font-medium'
                }`}
              >
